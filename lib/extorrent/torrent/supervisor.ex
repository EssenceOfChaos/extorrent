defmodule Extorrent.Torrent.Supervisor do
  @moduledoc """
    A torrent supervisor
  """
  use Supervisor
  ...
  def start_peer_manager(pid, info_hash, control_pid, file_worker) do
    child =
      supervisor(Extorrent.Peer.Manager, [info_hash, control_pid, file_worker],
        restart: :transient
      )
  ...
  def add_tracker(pid, info_hash, control_pid) do
    child = worker(Extorrent.Tracker, [info_hash, control_pid], restart: :transient)
  ...
  def start_file_worker(pid, info_hash, bitfield, control_pid) do
    child =
      worker(Extorrent.FileIO.Worker, [info_hash, bitfield, control_pid], restart: :transient)
  ...
  def init(info_hash) do
    children = [
      worker(Extorrent.Torrent.Control, [self(), info_hash], restart: :transient)
    ]
