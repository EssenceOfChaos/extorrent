defmodule Extorrent.Torrent.Control do
  @moduledoc """
  ...
  def handle_continue(:initialize, %{info_hash: info_hash} = state) do
    ...
    {:ok, num_pieces} = Table.get_num_pieces(info_hash)
  ...
  def handle_info({ref, bitfield}, %{info_hash: info_hash, sup_pid: sup_pid} = state) do
    ...
    Table.size_on_disk(info_hash, calculate_size_on_disk(bitfield, state))
    :ok = Table.release_check()
