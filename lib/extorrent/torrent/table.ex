defmodule Extorrent.Torrent.Table do
  @moduledoc """
    Manages Torrents and their data
  """
  @dialyzer [
  ...
  alias Extorrent.Torrent
  alias Extorrent.Torrent.Pool
  ...
  def handle_call({:add, path}, _from, %{torrents: torrents, downloads: downloads} = state) do
    case Torrent.parse_file(path) do
      {:ok, torrent} ->
        case Map.has_key?(torrents, torrent.info_hash) do
          false ->
            {:ok, pid} = Pool.add_torrent(torrent.info_hash, torrent.name)
