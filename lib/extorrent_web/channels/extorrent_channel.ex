defmodule ExtorrentWeb.ExtorrentChannel do
  @moduledoc false
  use Phoenix.Channel

  require Logger

  def join("extorrent:notifications", _message, socket) do
    torrents = Extorrent.Torrent.Table.subscribe()
    {payload, socket} = encode_info_hash(torrents, socket)
    {:ok, payload, socket}
  end

  def join("extorrent:" <> _topic, _params, _socket) do
